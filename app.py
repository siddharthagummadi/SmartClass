        ...
        if request.method == 'POST':
            ...
            if action == 'add':
                ...
                try:
                    ...
                    db.rollback()
                    flash('Error adding student: User or Roll No already exists.', 'error')
        ...
        if request.method == 'POST':
            ...
            if action == 'add':
                ...
                try:
                    ...
                    print(f"Error deleting teacher: {err}")
...
def train_model():
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': 'OpenCV Face module is missing. Use opencv-contrib-python-headless.'})
        ...
        report_data = [
            {
                'name': row[0],
                'rollNumber': row[1],
                'attendance': 'Present' if row[2] else 'Absent',
...
def get_student_attendance():
    ...
    percentage = round((result[1] / result[0]) * 100) if result and result[0] > 0 and result[1] is not None else 0
